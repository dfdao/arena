...
const provider = NETWORK.toString() == 'localhost'
    ? new ethers.providers.JsonRpcProvider(hardhat.httpRpc)
    : new ethers.providers.JsonRpcProvider(GNOSIS_OPTIMISM);
const pKey = NETWORK.toString() == 'localhost' ? process.env.DEV_PRIVATE_KEY : process.env.PROD_PRIVATE_KEY;
if (!pKey)
...
const sendDrip = async function (addresss) {
    ...
    await dripTx.wait();
    console.log(`${addresss} balance after drip`, ethers.utils.formatEther(await provider.getBalance(addresss)));
