    ...
    const requestAnimationFrame = (callback) => {
      ...
      const loop = () => {
        ...
        if (this.rendering === 'fps') {
          ...
          const now = Date.now();
          if (
            this.renderingPercentage > 0 &&
            now - lastRenderStart >=
              Math.min((lastRenderDuration * 100) / this.renderingPercentage, 1000)
          ) {
            lastRenderStart = now;
            execute();
...
if (df.performanceOptimizerState === undefined)
  df.performanceOptimizerState = new PerformanceOptimizerState();
...
class PerformanceOptimizer {
  ...
  async render(div) {
    $(div)
      ...
      .append(
        $('<div />')
          ...
          .append(
            $(
              `<input type="range" min="0" max="30" value="${state.renderingFps}" class="slider" id="fpsRange">`
            )
          )
      ...
      .append(
        $('<div />')
          ...
          .append(
            $(
              `<input type="range" min="0" max="100" value="${state.renderingPercentage}" class="slider" id="percentageRange">`
            )
          )
      ...
      .append(
        $('<div />')
          ...
          .append(
            $(
              `<input type="range" min="0" max="1000" value="${state.cacheLimit}" class="slider" id="cacheLimitSize">`
            )
          )
      ...
      .append(
        $('<div />').append(
          $(
            `<span class="${$("span[class^='Btn']:contains(save)").attr(
              'class'
            )}" id="cacheClearBtn">Clear</span>`
          )
        )
      );
    $(`input:radio[name=rendering][value=${state.rendering}]`).prop('checked', true);
    $('#cachingBox').prop('checked', state.caching);
    $('input:radio[name=rendering]').change(function () {
      state.rendering = this.value;
    });
    const [fpsSpan, percentageSpan, cacheLimitSizeSpan] = [
      $('#fpsSpan'),
      $('#percentageSpan'),
      $('#cacheLimitSizeSpan'),
    ];
    $('#fpsRange')
      .change(function () {
        state.renderingFps = parseInt(this.value);
      })
      .on('input', function () {
        fpsSpan.text(` ${this.value} FPS`);
      });
    $('#percentageRange')
      .change(function () {
        state.renderingPercentage = parseInt(this.value);
      })
      .on('input', function () {
        percentageSpan.text(` ${this.value}%`);
      });

    $('#cacheLimitSize')
      .change(function () {
        state.cacheLimit = parseInt(this.value);
      })
      .on('input', function () {
        cacheLimitSizeSpan.text(` ${this.value}`);
      });
    $('#cacheClearBtn').click(() => {
      this.clearSnarkCache();
    });
  }
